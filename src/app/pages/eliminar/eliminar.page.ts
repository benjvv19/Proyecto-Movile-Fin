...
export class EliminarPage implements OnInit {
  ...
  async verificarZapatilla() {
    // Buscar si existe una zapatilla
    const zapatilla = this.arregloZapatillas.find(z => z.nombre === this.nombre && z.id === parseInt(this.id, 10));

    if (zapatilla) {
      //alerta
      const alert = await this.alertController.create({
        header: 'Estas seguro?',
        message: `¿Deseas eliminar las zapatillas "${zapatilla.nombre}" con ID ${zapatilla.id}?`,
      ...
      const alert = await this.alertController.create({
        header: 'Error',
        message: 'No se encontró ninguna zapatilla con el nombre y ID proporcionados.',
        buttons: ['OK']
      });
      await alert.present();
    }
  ...
  eliminarZapatilla() {
    this.arregloZapatillas = this.arregloZapatillas.filter(z => !(z.nombre === this.nombre && z.id === parseInt(this.id, 10)));
  }
}
